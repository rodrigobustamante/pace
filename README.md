...
- **Training plan** — Goal-based day-by-day plan generated by Gemini; per-day willTrain toggle
...
Goal          # Active training goal (one per user)
TrainingPlan  # Gemini-generated plan linked to a goal
TrainingDay   # Individual day: workout type, duration, willTrain toggle
```
...
| `GET`      | `/api/coach/daily`              | Daily train/rest recommendation           |
| `POST`     | `/api/coach/chat`               | Streaming AI chat with conversation history |
| `GET`      | `/api/coach/activity/:id`       | Per-activity Gemini analysis              |
| `GET/POST` | `/api/goals`                    | Get active goal+plan / create new goal    |
| `DELETE`   | `/api/goals/:id`                | Delete goal (cascades to plan+days)       |
...
| **3** | 🔄 In progress | AI coach (weekly, daily, chat, per-activity) + goal-based training plan        |
